...
class PV_MainWindow( QtWidgets.QMainWindow ):
    ...
    def __init__( self, parent=None ):
        ...
        QtWidgets.QShortcut( QtGui.QKeySequence('return'), self.tree_view, self.toggle )
        QtWidgets.QShortcut( QtGui.QKeySequence('f'), self, self.fitPreviewImageToWindow )
        QtWidgets.QShortcut( QtGui.QKeySequence('Ctrl+1'), self, self.scale100 )
        QtWidgets.QShortcut( QtGui.QKeySequence('backspace'), self, self.delete )
    ...
    def updateCheckboxes( self, *args ):
        ...
        logger.debug('current: %s' % node.name )
    ...
    def getCheckedIndexes( self ):
        logger.debug('getCheckedIndexes()')
        model = self.tree_view.model()
    ...
    def copy( self, *args ):
        ...
        rel_paths = [node.dest_path for node in nodes]
        ...
        if not os.path.exists( folder ):
            logger.warn('Invalid path: %s' % folder )
            return
        ...
        src_paths = [node.path for node in nodes]
    ...
    def delete( self, *args ):
        ...
        for index in indexes:
            ...
            node = index.internalPointer()
            if isinstance( node, PV_ImageItem ):
                logger.debug('Deleting %s' % node.path )
                os.remove( node.path )
